...
function recordingNote(key) {
  ...
  if (!clip) return "";
  if (clip.readyExport) return `<p class="proof">${html(clip.id)} · ${html(clip.statusLabel)} · ${Math.round(clip.durationActual)} seconds. ${clip.previewUrl ? link(clip.previewUrl, "Watch edit") : ""}</p>`;
...
function recordings() {
  ...
  return `${intro("Your founder videos.", "You record. Codex handles the cuts, product visuals, sound and captions. Read one sentence at a time; pauses are useful.")}<div class="banner"><p><strong>Next: ${html(plan.startWith)}.</strong> Record vertically with the phone at eye level and a light in front of you. Send the original MOV or MP4 here.${first ? ` Send by ${date(first.recordBy, true)} for the ${date(first.targetAt, true)} target.` : ""}</p>${sourceLink(plan.guide, "Full recording guide")}</div><p class="help">All times are Chicago time. ${html(plan.status)}.</p><div class="list">${plan.clips.map((clip) => `<article class="work-row"><div>${campaignTag(clip.campaign)}<p>${tag(clip.statusLabel, ["ready", "scheduled", "published"].includes(clip.status) ? "success" : "neutral")}</p><div class="proof">${html(clip.id)} · ${clip.durationActual ? `${Math.round(clip.durationActual)} seconds` : html(clip.durationTarget)}</div></div><div><h2>${html(clip.title)}</h2><p>${html(clip.prompt)}</p><div class="meta"><span>${clip.receivedAt ? `Received ${date(clip.receivedAt, true)}` : `Send by ${date(clip.recordBy, true)}`}</span><span> · Target ${date(clip.targetAt, true)}</span></div>${clip.previewUrl ? `<p>${link(clip.previewUrl, "Watch finished edit")}</p>` : ""}<p class="proof">${html(clip.targetKind)}${clip.replaceBy ? ` · Replacement cutoff ${date(clip.replaceBy, true)}` : ""}</p>${clip.script ? `<details ${clip.id === plan.startWith ? "open" : ""}><summary>${clip.sourceRecording ? "Original recording prompt" : "Read this aloud"}</summary>${clip.script.split("\n\n").map((line) => `<p>${html(line)}</p>`).join("")}</details>` : sourceLink(clip.scriptSource, "Current film script")}<p><strong>The edit:</strong> ${html(clip.edit)}</p><p class="proof">${html(clip.fallback)}</p></div></article>`).join("")}</div><div class="note"><strong>Ready means edited and checked.</strong><p>Codex combines your actual voice and camera with the existing images and product footage. We keep short captions readable and sample labels visible. A late recording can become a later story. A saved video schedule still needs separate publication evidence.</p><p>Plan updated ${date(plan.updatedAt, true)}. This page reads saved records; sending a file here does not update it automatically.</p></div>`;
}
